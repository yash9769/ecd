...
export default function Hero() {
  ...
  return (
    <section
      ref={heroRef}
      className="relative overflow-hidden bg-white transition-colors duration-300 dark:bg-[#0e0822] md:min-h-screen md:flex md:flex-col md:justify-center"
    >
      {/* Top-Left Subtle Dot Grid Pattern */}
      <div
        aria-hidden="true"
        className="pointer-events-none absolute left-[2%] top-[10%] hidden h-[100px] w-[100px] opacity-30 lg:block dark:opacity-20"
        style={{
      ...
      {/* Cyber aura blob (Dark Mode Only) */}
      <div
        aria-hidden="true"
      ...
      />
      {/* Aurora ambient layer (Dark Mode Only) */}
      <div className="aurora-bg hidden dark:block" aria-hidden="true" />

      {/* Bottom right atmospheric gradient (Dark Mode Only) */}
      <div
        aria-hidden="true"
        className="pointer-events-none absolute bottom-0 right-0 h-[60%] w-[40%] opacity-0 transition-opacity duration-300 dark:opacity-20"
        style={{
      ...
      <div className="relative mx-auto w-full max-w-[1380px] px-5 pt-28 pb-10 sm:px-6 sm:pt-32 md:pt-20 md:pb-6 lg:px-10 lg:pt-24 lg:pb-8">
        ...
        >
          ...
          <div className="max-w-[34rem]" style={{ gridArea: "text" }}>
            {/* Kicker */}
            <div className="hero-kicker font-mono text-[11px] font-bold uppercase tracking-[0.22em] text-[#4f46e5] dark:text-[#818cf8] lg:text-[11.5px]">
              ANTICIPATE. PROTECT. OUTPERFORM.
            </div>

            {/* Headline */}
            <h1 className="hero-h1 mt-3 font-display text-[clamp(34px,8vw,54px)] font-extrabold leading-[1.05] tracking-[-0.04em] text-[#0d1020] dark:text-white lg:mt-4 transition-colors duration-300">
            ...
            {/* Subtitle */}
            <p className="hero-subtitle mt-3 max-w-md text-[15px] font-semibold leading-snug text-[#1e293b] dark:text-slate-200 sm:text-[16px] lg:mt-3.5 lg:text-[17px] transition-colors duration-300">
              Your Strategic cybersecurity for what&rsquo;s next.
            </p>

            {/* Paragraph */}
            <p className="hero-body mt-2.5 max-w-md text-[13.5px] leading-relaxed text-[#575f75] dark:text-slate-400 lg:text-[14.5px] transition-colors duration-300">
          ...
          <div style={{ gridArea: "visual" }} className="hero-visual flex justify-center md:justify-end">
            <HeroVisual />
          </div>
        </div>
      </div>
